...
class StockPredictorAPI:
    ...
    def predict_next_day(self, ticker):
        ...
        try:
            ...
            return {
                ...
                "prediction_date": (dt.datetime.now() + dt.timedelta(days=1)).strftime("%Y-%m-%d"),
    ...
    def _generate_explanation(self, current_price, predicted_price):
        ...
        latest_macd = self.df['MACD'].iloc[-1] if not pd.isna(self.df['MACD'].iloc[-1]) else 0
        
        trend_direction = "bullish" if change_percent > 0 else "bearish"
        confidence = min(abs(change_percent) * 10, 85)  # Cap confidence at 85%
